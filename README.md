...
```json
{
    "log": {
        "loglevel": "warning",
        "_comment_loglevel": "日志级别：warning 只记录警告及以上日志，适合生产环境，以控制日志大小。"
    },
    "inbounds": [
        {
            "listen": "0.0.0.0",
            "_comment_listen": "监听地址：0.0.0.0 表示监听所有网络接口和公网 IP。",
            "port": 14559,
            "_comment_port": "监听端口：VLESS 接收连接的端口。由于您的限制，使用了非标准 HTTPS 端口。",
            "protocol": "vless",
            "_comment_protocol": "协议类型：VLESS，一种轻量级、高性能的传输协议。",
            "settings": {
                "clients": [
                    {
                        "id": "f76f4404-73a0-439e-b449-65ddca9d8614",
                        "_comment_id": "客户端 UUID：用于身份验证。客户端必须使用此 ID 连接。"
                    }
                ],
                "decryption": "none",
                "_comment_decryption": "VLESS 解密方式：必须为 none，因为 REALITY 在底层提供安全，VLESS 数据体不应额外加密。"
            },
            "streamSettings": {
                "network": "tcp",
                "_comment_network": "底层网络协议：REALITY 目前必须运行在 TCP 上。",
                "security": "reality",
                "_comment_security": "传输安全：使用 REALITY 机制，将流量伪装成访问真实网站的 HTTPS 流量。",
                "tcpSettings": {
                    "header": {
                        "type": "none",
                        "_comment_type": "TCP 伪装头类型：none 表示不使用额外的 TCP 伪装，依赖 REALITY 本身的伪装能力。"
                    }
                },
                "realitySettings": {
                    "show": false,
                    "_comment_show": "显示握手信息：false 表示不在日志中显示 REALITY 握手信息，增强隐蔽性。",
                    "dest": "server8.webhostmost.com:443",
                    "_comment_dest": "目标网站：伪装流量要访问的公网真实网站地址和端口。",
                    "handshake": null,
                    "_comment_handshake": "握手目标：高级设置，通常留空或 null。",
                    "session": null,
                    "_comment_session": "会话 ID：高级设置，通常留空或 null。",
                    "privateKey": "KKskeZ3BJ-ZroZ6w9dMEP-aM1ZA0GQasvZXFJ6ckR04",
                    "_comment_privateKey": "服务器私钥：用于 REALITY 握手和验证，必须保密，客户端使用对应的公钥。",
                    "minClientVer": "",
                    "_comment_minClientVer": "最小客户端版本：留空表示不限制。",
                    "maxClientVer": "",
                    "_comment_maxClientVer": "最大客户端版本：留空表示不限制。",
                    "maxSha256Time": 60,
                    "_comment_maxSha256Time": "随机数有效期：REALITY 握手随机数（Salt）的最长有效时间（秒），用于防重放攻击。",
                    "fingerprints": [
                        "chrome",
                        "firefox"
                    ],
                    "_comment_fingerprints": "客户端指纹：允许客户端伪装的 TLS 浏览器指纹列表，提高抗探测能力。",
                    "serverNames": [
                        "server8.webhostmost.com"
                    ],
                    "_comment_serverNames": "服务器名称 (SNI)：伪装的域名，客户端必须用此 SNI 连接，且通常需与 dest 网站匹配。",
                    "shortIds": [
                        "fe4f754a504e7cac"
                    ],
                    "_comment_shortIds": "短 ID 列表：额外的身份验证 ID，客户端必须提供其中一个。",
                    "spiderX": "/",
                    "_comment_spiderX": "路径伪装：客户端伪装访问的 URL 路径。"
                }
            }
        }
    ],
    "outbounds": [
        {
            "protocol": "freedom",
            "_comment_protocol_out": "出站协议：freedom 表示直接将流量转发到目标网站。",
            "settings": {},
            "_comment_settings_out": "出站设置：freedom 协议通常不需要特殊设置。"
        }
    ],
    "_comment_FileEnd_": "配置结束。"
}
```
